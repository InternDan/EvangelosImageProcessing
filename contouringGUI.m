...
elseif strcmpi(handles.analysis,'EvangelosToothThicknessQuadrants') == 1
    [hObject, eventdata,handles] = EvangelosToothThicknessQuadrants(hObject,eventdata,handles);
elseif strcmpi(handles.analysis,'SetCurrentSliceToWidest') == 1
    [hObject, eventdata,handles] = SetCurrentSliceToWidest(hObject,eventdata,handles);
...
elseif strcmpi(handles.primitive,'rectangle') == 1
    ...
    guidata(hObject, handles);
elseif strcmpi(handles.primitive,'line') == 1
    set(handles.textBusy,'String','Busy');
    guidata(hObject, handles);
    drawnow();
...
%     %a = a*-1;
%     
    [a1 b1] = size(handles.img(:,:,handles.slice));
%     
%     
%     for i = 1:b1
%         lineOut(i) = round(a*i+b);
%     end
    ...
    BW = false(size(handles.img(:,:,handles.slice)));
    for i = 1:b1
        theVal = round(val+slope*i);
        if theVal <= a1 && theVal >= 1
            BW(theVal,i) = 1;
        end
    end
    BW = imdilate(BW,true(7,7));
    
   
    if isfield(handles,'bwContour')
        bw2 = handles.bwContour(:,:,handles.slice);
        bw2(BW) = 1;
        handles.bwContour(:,:,handles.slice) = bw2;
    else
        handles.bwContour = false(size(handles.img));
        bw2 = handles.bwContour(:,:,handles.slice);
        bw2(BW) = 1;
...
switch str{val}
   ...
   %     handles.analysis = 'GuilakKneeSurface';
    ...
    case 'EvangelosToothThicknessQuadrants'
        handles.analysis = 'EvangelosToothThicknessQuadrants';
    case 'SetCurrentSliceToWidest'
        handles.analysis = 'SetCurrentSliceToWidest';
